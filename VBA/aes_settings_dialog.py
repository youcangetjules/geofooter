...
CLI:
  pythonw aes_settings_dialog.py --accounts accounts.json --out result.json
...
""".replace(
    "__CHEVRON__", (Path(__file__).resolve().parent / "icons" / "chevron_down.svg").as_posix()
)
...
def probe_abuseipdb_endpoint(
    ...
    try:
        from aes_secrets import aes_http_get, abuseipdb_check_url as _default_check_url
    except ImportError:
...
def run_dialog(
    ...
    try:
        from aes_settings_panels import build_trusted_senders_panel
    ...
    # ---- Tab 3: External APIs ----
    try:
        from aes_secrets import abuseipdb_status, get_abuseipdb_base_url
    except ImportError:
    ...
    try:
        from aes_settings_panels import build_threat_intel_panel
