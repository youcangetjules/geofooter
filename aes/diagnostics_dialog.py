...
CLI:
  pythonw aes\\diagnostics_dialog.py --context context.json
...
def test_geolocate_file(_ctx: DiagContext) -> TestResult:
    path = str(_suite_root() / "aes" / "geolocate_headers.py")
    return _pass_fail(
        _exists(path),
        "aes\\geolocate_headers.py found",
        "aes\\geolocate_headers.py not found",
        path,
    )


def test_guri_file(_ctx: DiagContext) -> TestResult:
    path = str(_suite_root() / "guri" / "core.py")
    return _pass_fail(_exists(path), "guri\\core.py found", "guri\\core.py not found", path)
...
_VENV_ROOT = _suite_root() / ".venv"
_REQUIREMENTS_PATH = _suite_root() / "requirements.txt"
...
def build_tests() -> List[TestDef]:
    ...
    return [
        ...
        TestDef("geolocate", "geolocate_headers.py", "AES scanner", "install_root/aes/geolocate_headers.py", test_geolocate_file),
        TestDef("guri", "guri core", "AES scanner", "install_root/guri/core.py", test_guri_file),
