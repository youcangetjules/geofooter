...
Public Sub HandleNewMailEx(ByVal entryIdCollection As String)
    ...
    For i = LBound(parts) To UBound(parts)
        ...
        If itm Is Nothing Then GoTo NextNewMailPart
        If Not MSCANModule1.IsScannableItem(itm) Then GoTo NextNewMailPart
        ' Account filter before any body/header work — HTMLBody freezes Outlook.
        If Not MSCANSettings.IsMailItemAccountScanEnabled(itm) Then GoTo NextNewMailPart
        ' Footer check deferred to ProcessEmail (reads HTMLBody).
...
NextNewMailPart:
    ...
    If queued > 0 Then
        MSCANModLogging.WriteLog "NewMailEx: queued " & queued & " item(s)."
    End If
    Exit Sub
