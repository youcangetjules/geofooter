...
# Allow imports from VBA subfolder (script) and GeoFooter project root (guri.py, etc.)
_SCRIPT_DIR = Path(__file__).resolve().parent
_GEOFOOTER_ROOT = _SCRIPT_DIR.parent if _SCRIPT_DIR.name.lower() == "vba" else _SCRIPT_DIR
for _path in (_SCRIPT_DIR, _GEOFOOTER_ROOT):
    _path_str = str(_path)
...
try:
    from link_scanner import LinkScanner, LinkFinding
except ImportError:
    LinkScanner = None  # type: ignore
    LinkFinding = None  # type: ignore

try:
    from body_scanner import BodyScanner, BodyScanResult
except ImportError:
    BodyScanner = None  # type: ignore
    BodyScanResult = None  # type: ignore

try:
    from aes_secrets import (
        abuseipdb_check_url,
        aes_http_get,
...
CONFIG = {
    ...
    "api_keys": {
        # AbuseIPDB is loaded from DPAPI-secured settings (see aes_secrets.py).
    ...
    def _check_blocklists(self, ip: str) -> str:
        ...
        try:
            ...
            for bl in blocklists:
                ...
                if query and self._check_single_blocklist(query):
                    listed_in.append(bl)
...
class SecurityAssessor:
    ...
    def apply_route_path_risk(
        ...
        self._record_factor(
            ...
            15,
            note=f"AbuseIPDB ≥ {threshold} or listed ASN; +15 once",
        )
        score = max(0, min(100, int(assessment.score) + 15))
        ...
        if origin_geo.success:
                ...
                if abuse_score >= threshold:
                    ...
                    self._record_factor(
                        ...
                        note=(
                            "Damped by whitelist"
                            if whitelisted
                            else "Same threshold as high-risk route hop"
                        ),
                    )
                    score += pts
                    geo_penalty += pts
            ...
            if no_data_count > 0:
                ...
                self._record_factor(
                    ...
                    pts,
                    note="+2 per Tor/Proxy/Anonymous/VPN with no data",
                )
                score += pts
    ...
    def _assess_domain_age(
        ...
        if not whois_info or "error" in whois_info:
            if whois_info and whois_info.get("error"):
                self._record_factor(
                    factors, components, cat, "Sender Domain WHOIS Error", 5
                )
                score += 5
...
class HTMLReportGenerator:
    ...
    def _prepare_display_data(
        ...
        attachment_scan_results = self._scan_exported_attachments(metadata, deep_mode=deep_mode)
        ...
        # Body is exported for every scan now, so link safety heuristics run
        # in Basic scans too (deep-scan risk fusion stays deep-only).
        ...
        action_buttons_html = self._build_action_buttons_html(
            sender_email, sender_domain, guri, block_state, links_report_url
        )
        ...
        created_zulu = datetime.now(pytz.utc).strftime('%Y %m %d %H:%M') + ' Zulu'
        ...
        if links_summary_line:
            summary_line1_html = f"{summary_line1_html} | {links_summary_line}"
        ...
        summary_line2_prefix = (
            ...
            f"Sent: {sender_local_plain} sender-local | "
    ...
    def _format_sender_local_time(
        ...
        note_html = (
            f" <span style='color: #888888;'>({html.escape('; '.join(notes))})</span>"
            if notes else ""
        )
        html_out = f"<span style='color: {colour};'>{html.escape(plain)}</span>{note_html}"
    ...
    def _sender_block_state(
        ...
        return {
            "attachments": on_list("block_attachments"),
            "beacons": on_list("block_beacons"),
            "trusted": on_list("trusted"),
            "untrusted": on_list("untrusted"),
        }
    ...
    def _render_status_banner_png(
        ...
        Left edge is a colour square for the risk band:
          0–25 green, 25–50 amber, >50 red.
        Text starts to the right of that square so it never overlaps.
        Returns the PNG's local path, or '' on failure.
        """
        try:
        ...
        try:
            ...
            border = (230, 194, 0) if warn else (207, 216, 220)     # #e6c200 / #cfd8dc

            font_path = Path(os.environ.get("WINDIR", r"C:\Windows")) / "Fonts" / "arial.ttf"
            try:
                font = ImageFont.truetype(str(font_path), 14)
                score_font = ImageFont.truetype(str(font_path), 11)
            except Exception:
                font = ImageFont.load_default()
                score_font = font

            # Risk square on the left; text inset so it never overlaps.
            square = 26
            gap = 8
            ...
            pad_y = 4
            width = max(720, text_left + text_w + 12)
            height = max(square + 2, text_h + pad_y * 2 + 2)

            img = Image.new("RGB", (width, height), bg)
            draw = ImageDraw.Draw(img)
            draw.rectangle([0, 0, width - 1, height - 1], outline=border, width=1)

            # Left risk square (flush to the left edge inside the border).
            sq_top = (height - square) // 2
            draw.rectangle(
                [1, sq_top, square, sq_top + square - 1],
                fill=square_rgb,
                outline=square_rgb,
            )
            # Score numeral centred in the square (white).
            score_txt = str(score)
            sb = draw.textbbox((0, 0), score_txt, font=score_font)
            sw, sh = sb[2] - sb[0], sb[3] - sb[1]
            draw.text(
                (1 + (square - 1 - sw) // 2 - sb[0], sq_top + (square - sh) // 2 - sb[1]),
                score_txt,
                fill=(255, 255, 255),
                font=score_font,
            )
    ...
    def _build_top_of_mail_banners(
        ...
        *,
        ...
        att_blocked = bool(block_state.get("attachments") or attachments_quarantined > 0)
        bcn_blocked = bool(
        ...
        elif att_blocked and bcn_blocked:
            sender_status = "A&B Blocked"
        elif att_blocked:
            sender_status = "A Blocked"
        elif bcn_blocked:
            sender_status = "B Blocked"
        elif untrusted:
            sender_status = "not trusted"
        else:
            sender_status = "unknown"

        link_total = len(link_findings or [])
        highs = sum(
            1 for f in (link_findings or []) if str(field(f, "risk_level")) == "high"
        )
        meds = sum(
            1 for f in (link_findings or []) if str(field(f, "risk_level")) == "medium"
        )
        hre = highs + meds + int(beacon_count or 0) + int(attachment_not_ok or 0)
        score = max(0, min(100, int(risk_score or 0)))
        level = (risk_level or "").strip().upper() or (
            "LOW" if score <= 25 else ("MEDIUM" if score <= 50 else "HIGH")
        )
        ...
        line = (
            ...
            f"A: {int(attachment_count or 0)}; "
            f"B: {int(beacon_count or 0)}; "
            f"L: {link_total}; "
            f"HRE: {hre}"
        )
        if hre > 0:
        ...
        warn = hre > 0
        img_path = self._render_status_banner_png(line, warn=warn, risk_score=score)
        if img_path:
            ...
            return (
                ...
                f"<div style='margin:0 0 4px 0; line-height:0;'>"
                f"<img src='{html.escape(img_url, quote=True)}' "
                f"width='720' style='display:block; width:100%; max-width:720px; "
                f"height:auto; border:0; outline:none;' alt='' /></div>\n"
                "<!-- AES Top Banners End -->\n"
            )

        if warn:
            bg, fg, border = "#fff8e6", "#5d4037", "#e6c200"
        else:
            bg, fg, border = "#f4f7f9", "#37474f", "#cfd8dc"
        sq = (
            "#4CAF50" if score <= 25 else ("#FFA726" if score <= 50 else "#E53935")
        )
        return (
            "<!-- AES Top Banners Start -->\n"
            f"<div style='margin:0 0 4px 0; padding:4px 8px 4px 36px; text-align:left; "
            f"font-family:Arial,sans-serif; font-size:11px; line-height:1.35; "
            f"color:{fg}; background:{bg}; border:1px solid {border}; position:relative;'>"
            f"<span style='position:absolute; left:4px; top:4px; width:22px; height:22px; "
            f"background:{sq}; color:#fff; font-size:10px; font-weight:bold; "
            f"text-align:center; line-height:22px;'>{score}</span>"
    ...
    def _write_links_report_file(
        ...
        try:
            ...
            scanned_at = datetime.now(pytz.utc).strftime("%Y-%m-%d %H:%M:%S Zulu")
            summary_colour = (
...
<div class="footer">(C) Aliniant Labs | Aliniant Email Scanner (AES)</div>
    ...
    def _build_action_buttons_html(
        ...
        if not sender and (not domain or domain == "Unknown") and not links_report_url:
            return ""
        ...
        def style(active: bool, active_bg: str) -> str:
            if active:
                return (
                    f"display:inline-block; padding:3px 10px; margin:2px 6px 2px 0; "
                    f"border:1px solid {active_bg}; border-radius:3px; color:#ffffff; "
                    f"font-size:10px; font-weight:bold; text-decoration:none; background:{active_bg};"
                )
            return (
                "display:inline-block; padding:3px 10px; margin:2px 6px 2px 0; "
                "border:1px solid #0f6b7c; border-radius:3px; color:#0f6b7c; "
                "font-size:10px; font-weight:bold; text-decoration:none; background:#f2f8fa;"
            )
            ...
            buttons.extend(
                [
                    ...
                    (
                        ...
                        "Sender TRUSTED &#10003;" if trusted else (
                            "Trust sender (undoes blocks)" if (att_on or bcn_on or untrusted) else "Trust sender"
                        ),
                        style(trusted, "#2e7d32"),
                    ),
                    (
                        f"aes://untrust-sender?{query}",
                        "Sender NOT TRUSTED &#10003;" if untrusted else "Mark not trusted",
                        style(untrusted, "#ef6c00"),
                    ),
                ]
            )
        if not buttons:
            return ""
        links = "".join(
            f"<a href='{html.escape(url)}' style='{btn_style}'>{label}</a>"
            for url, label, btn_style in buttons
        )
        return (
            "<div style='margin-top:6px; padding-top:4px; border-top:1px solid #ddd;'>"
            "<span style='font-size:10px; color:#666; margin-right:6px;'>AES actions:</span>"
            f"{links}</div>"
        )
    ...
    def _scan_exported_attachments(
        ...
        scanner = AttachmentScanner(deep_mode=deep_mode)
        results = scanner.scan_directory(scan_dir)
        self.logger.info(
            "Attachment scan %s (deep=%s): %d file(s), %d OK, %d Not OK",
            scan_dir,
            deep_mode,
            len(results),
    ...
    def _load_deep_body_payload(self, metadata: Dict[str, str]) -> Tuple[str, str]:
        """Load body.html / body.txt sidecars exported for Deep Scan."""
    ...
    def _aes_subtle_link(
        ...
        *,
        ...
        """Footer link styled as plain text (real href, no box or underline)."""
        return (
            f'<a href="{html.escape(href, quote=True)}" target="_blank" '
            f'style="color:#ffffff;text-decoration:none;background:transparent;border:none;'
    ...
    def _create_attachment_assets(
        ...
        return self._aes_subtle_link(report_url, inner), report_url
...
<div class="footer">(C) Aliniant Labs | Aliniant Email Scanner (AES)</div>
    ...
    def _create_beacon_assets(
        ...
        label = f"{beacon_count} beacon{'s' if beacon_count != 1 else ''} detected"
        if beacon_blocked is not None:
            label = f"{label} / {beacon_blocked} blocked"
        return self._aes_subtle_link(report_url, f"Beacons: {html.escape(label)}"), report_url

    def _ensure_aes_static_pages(self) -> None:
        """Ensure fallback pages exist for beacon viewing without AES installed."""
        pages_dir = Path(CONFIG["base_path"]) / "pages"
...
<p><a href="{report_url}">Click here if you are not redirected</a></p>
    ...
    def _infer_sender_domain(
        ...
        for hop in reversed(per_hop_analysis or []):
    ...
    def _build_html(self, data: Dict[str, Any], footer_mode: str = "compact") -> str:
        """Build AES footer HTML. compact = two-line summary only; full = summary + detailed analysis."""
        footer_mode = (footer_mode or "compact").lower()
        ...
        if risk_report_url:
            risk_html = self._aes_subtle_link(
                risk_report_url, risk_label, font_weight="bold", font_size="12px"
            )
        else:
        ...
        if footer_mode == "full":
            ...
            report_url = self._write_full_scan_report_file(data, full_details, risk, score)
            if report_url:
                show_link = self._aes_subtle_link(report_url, "Show Full Scan")
            else:
        ...
        if footer_mode == "compact":
            return (
                f"{top_banners}"
                f"<!-- AES Start -->\n"
                f"<div style='margin:0; padding:2px; background:#fff; font-family:Arial, sans-serif; "
        ...
        *,
        ...
        for item in components:
            ...
            rows.append(
                ...
                f"{point_style}'>+{points}</td>"
                f"<td style='padding:6px 8px;border-bottom:1px solid #eee;color:#666;'>"
        ...
        thresholds = CONFIG["risk_thresholds"]
        high_at = thresholds["low"]
        medium_at = thresholds["medium"]
        ...
        return (
            ...
            f"<li>LOW: score &lt; {medium_at}</li>"
            f"<li>MEDIUM: score ≥ {medium_at} and &lt; {high_at}</li>"
            f"<li>HIGH: score ≥ {high_at}, or a domain-age caution flag is set</li>"
            f"</ul>"
            ...
            f"<li>Deep Scan: links/body/attachments/beacons add capped points when that mode runs</li>"
...
<div class="footer">(C) Aliniant Labs | Aliniant Email Scanner (AES) Deep Scan — ribbon only, never auto-run</div>
    ...
    def _build_route_svg_map_html(self, hops: List[Dict[str, Any]]) -> str:
        ...
        for idx, hop in enumerate(hops or [], 1):
            ...
            if lat is None or lon is None or hop.get("hop_kind") == "internal":
                continue
            try:
                lat_f, lon_f = float(lat), float(lon)
            except (TypeError, ValueError):
                continue
            label = f"#{idx} {hop.get('city') or ''} {hop.get('country') or ''} ({hop.get('ip') or ''})"
            points.append((lon_f, lat_f, idx, label.strip()))
    ...
    def _build_hop_map_html(self, hops: List[Dict[str, Any]]) -> str:
        ...
        for idx, hop in enumerate(hops or [], 1):
            ...
            try:
                ...
                continue
            label = html.escape(
                f"#{idx} {hop.get('city') or ''}, {hop.get('country') or ''} — {hop.get('ip') or ''}"
            )
            points.append({"lat": lat_f, "lon": lon_f, "label": label, "n": idx})
...
(function() {{
  ...
  points.forEach(function(p) {{
    var ll = [p.lat, p.lon];
    latlngs.push(ll);
    L.marker(ll).addTo(map).bindPopup(p.label);
  }});
  if (latlngs.length > 1) {{
...
<div class="footer">(C) Aliniant Labs | Aliniant Email Scanner (AES) — already calculated at scan time (no re-scan)</div>
    ...
    def _build_full_details_html(
        ...
        if data.get('per_hop_analysis'):
            ...
            for h in headers:
                per_hop_html += f"<th style='border-bottom:1px solid #ccc; padding:2px 4px; text-align:left; font-size:10px; color:#333; font-weight:600; word-break:break-word; overflow-wrap:anywhere;'>{h}</th>"
            per_hop_html += "</tr>"
            for idx, hop in enumerate(data['per_hop_analysis'], 1):
                ...
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{idx}</td>"
                ...
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{host_disp}</td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{html.escape(str(hop.get('ip') or 'Unknown'))}</td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{html.escape(str(hop.get('asn') or 'Unknown'))}</td>"
                ...
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{asn_org_disp}</td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{hop['prefix']}</td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{hop['rir']}</td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{hop['country']}<br><span style='color:{row_color}; font-size:9px; font-weight:{row_weight};'>{hop['city']}</span></td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{hop['reverse_dns']}</td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{hop['reputation']}</td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{hop['timestamp']}</td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{hop['delay']}</td>"
                per_hop_html += f"<td style='padding:2px 4px; word-break:break-word; overflow-wrap:anywhere; color:{row_color}; font-weight:{row_weight};'>{', '.join(hop['bgp_peers']) if hop['bgp_peers'] else ''}</td>"
                per_hop_html += "</tr>"
            per_hop_html += "</table>"
            # GURI, copyright, datetime at the bottom
            guri = data.get('guri', 'N/A')
            now = datetime.now(pytz.utc)
            created_zulu_str = data.get('created_zulu', now.strftime('%Y %m %d %H:%M') + ' Zulu')
    ...
    def _decode_m365_host_label(self, label: str) -> Optional[str]:
        ...
        m = re.match(r'^([A-Z]{2})(\d)(PEPF)(\d+)([A-Z]*)$', raw)
        if m:
            dc, ring, role, num, _tail = m.groups()
            city = self._M365_DC_CODES.get(dc, dc)
            role_name = self._M365_ROLE_CODES.get(role, role)
            return f"{city} / {role_name} #{num} (ring {ring})"

        # Backend-proxy form without env letter: AMBP250MB1516
        m = re.match(r'^([A-Z]{2})(BP)(\d+)(MB|CA|WS|FE|BE|EP|DL)(\d+)$', raw)
        if m:
            dc, bp, forest, role, num = m.groups()
            city = self._M365_DC_CODES.get(dc, dc)
            role_name = self._M365_ROLE_CODES.get(role, role)
            return (
                f"{city} / {self._M365_ROLE_CODES['BP']} / "
                f"forest {forest} / {role_name} #{num}"
            )

        # Classic form: AM0PR10CA0036 / LO4P123CA0189 / DU0P250MB0793
        m = re.match(
            r'^([A-Z]{2})(\d)(PR|P|SD|T)(\d+)(CA|MB|WS|FE|BE|EP|BP|DL)(\d+)$',
            raw,
        )
        if m:
    ...
    def _decode_m365_hostname(self, hostname: Optional[str]) -> Optional[str]:
        ...
        parts = host.split(".")
        bits: List[str] = []
        first = self._decode_m365_host_label(parts[0])
        if first:
            bits.append(first)
        if len(parts) > 1:
    ...
    def _extract_hop_details(self, headers: Optional[str]) -> list:
        ...
        for line in received_lines:
            ...
            if hop['fqdn']:
                hop['decoded_location'] = self._decode_m365_hostname(hop['fqdn'])
            if not hop['decoded_location'] and hop['by_host']:
                hop['decoded_location'] = self._decode_m365_hostname(hop['by_host'])
            ...
            note_parts: List[str] = []
            if hop['by_host']:
                note_parts.append(f"by {hop['by_host']}")
            if hop['helo']:
            ...
            if public_ip:
                ...
                hop['lat'] = geo.lat
                hop['lon'] = geo.lon
                ipwhois_info = self._ipwhois_lookup(public_ip)
                if hop['asn'] == 'Unknown' and ipwhois_info.get('asn'):
                    hop['asn'] = ipwhois_info['asn']
                if hop['asn_org'] == 'Unknown' and ipwhois_info.get('org'):
                    hop['asn_org'] = ipwhois_info['org']
                hop['prefix'] = ipwhois_info.get('prefix', '—') or '—'
                hop['rir'] = ipwhois_info.get('rir', '—') or '—'
                try:
                    hop['reverse_dns'] = socket.gethostbyaddr(public_ip)[0]
                except Exception:
