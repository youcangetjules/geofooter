...
# Launched by path (ribbon / VBA / .bat): put the install root (not guri\) on sys.path.
...
def _datastore_file(name: str) -> str:
    """User list/state files (recipients, important domains, reply status)."""
    try:
        from geofooter.paths import datastore_path

        return str(datastore_path(name))
    except Exception:
        return str(_PKG_DIR.parent / "datastore" / name)


from guri.core import GURIDatabase, connect_guri_database
from guri.decompositor import GURIDecompositor
from guri.component_library import GURIComponentLibrary
from guri.action_detector import (
    CATEGORY_ACTIONABLE,
    CATEGORY_DEPRECATED,
    CATEGORY_IMPORTANT,
    CATEGORY_LESS,
...
from guri.outlook_scraper import (
...
from guri.inbox_search import (
    load_inbox_search_settings,
    save_inbox_search_settings,
    search_inbox_conversations,
)
import aes.score_history as aes_score_history
from aura.gui_tab import DataBrokersPanel
from guri.file_scraper import (
    ScanLocation,
...
from guri.ollama_client import (
...
class GURIViewerGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        try:
            ...
            from guri.learning import normalize_learning
    ...
    def _save_install_root(self) -> None:
        ...
        if not raw:
            ...
            return
        try:
            from geofooter.paths import set_install_root
    ...
    def _connect_database_default(self):
        ...
        try:
            ...
            QMessageBox.critical(
                ...
                f"{e}\n\n"
                "Create guri_postgres_config.json in the install root from the example file, "
...
"""
    ...
    def _load_recipients(self):
        ...
        recipients_file = _datastore_file("recipients.txt")
    ...
    def _load_important_domains(self):
        ...
        domains_file = _datastore_file("important_domains.txt")
    ...
    def _load_email_status(self):
        ...
        status_file = _datastore_file("email_status.json")
    ...
    def _preview_toggle_vip_sender(self) -> None:
        ...
        if not addr:
            ...
            return
        from guri.learning import is_important_sender
...
"""
    ...
    def _apply_category_correction(
        ...
        *,
        ...
        from guri.learning import category_ladder_index
        ...
        if moving_down:
            ...
            if applied in {CATEGORY_ACTIONABLE, CATEGORY_IMPORTANT}:
                from guri.action_store import endorse_sender_score
...
def main():
    ...
    """
    try:
        from geofooter.paths import get_install_root, set_install_root, user_data_dir
        ...
        from geofooter.crashlog import install_sys_excepthook
