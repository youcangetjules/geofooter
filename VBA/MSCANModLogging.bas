...
Public Sub WriteLogLevel(ByVal level As String, ByVal msg As String)
    ...
    line = Format$(Now, "yyyy-mm-dd HH:nn:ss") & " | " & lvl & " | " & msg
    ...
    If mCaptureActive And Len(mCapturePath) > 0 Then
        ...
        f = FreeFile
        Open mCapturePath For Append As #f
        Print #f, line
        Close #f
        On Error GoTo EH
    End If
...
'------------------------------------------------------------------------
' Diagnostics / settings: persist levels, capture sidecar
'------------------------------------------------------------------------

Public Sub SaveLoggingSettings( _
    ByVal enabled As Boolean, _
    ByVal infoOn As Boolean, _
    ByVal auditOn As Boolean, _
    ByVal warnOn As Boolean, _
    ByVal debugOn As Boolean _
)
    On Error GoTo EH
    ...
    ts.WriteLine "{"
    ts.WriteLine "  ""enabled"": " & IIf(enabled, "true", "false") & ","
    ts.WriteLine "  ""levels"": {"
    ts.WriteLine "    ""info"": " & IIf(infoOn, "true", "false") & ","
    ts.WriteLine "    ""audit"": " & IIf(auditOn, "true", "false") & ","
    ts.WriteLine "    ""warn"": " & IIf(warnOn, "true", "false") & ","
    ts.WriteLine "    ""debug"": " & IIf(debugOn, "true", "false")
    ts.WriteLine "  }"
    ts.WriteLine "}"
    ts.Close
    ...
    mLevelDebug = debugOn
    mLoggingLoaded = True
    WriteLogAudit "Logging settings saved (info=" & infoOn & " audit=" & auditOn & _
        " warn=" & warnOn & " debug=" & debugOn & " enabled=" & enabled & ")"
    Exit Sub
EH:
    Debug.Print "SaveLoggingSettings failed: " & Err.Number & " - " & Err.Description
End Sub

Public Function GetLoggingSettingsJson() As String
    On Error Resume Next
    EnsureLoggingSettingsLoaded
    GetLoggingSettingsJson = _
        "{""enabled"":" & IIf(mLoggingEnabled, "true", "false") & _
        ",""levels"":{""info"":" & IIf(mLevelInfo, "true", "false") & _
        ",""audit"":" & IIf(mLevelAudit, "true", "false") & _
        ",""warn"":" & IIf(mLevelWarn, "true", "false") & _
        ",""debug"":" & IIf(mLevelDebug, "true", "false") & "}" & _
        ",""capture_active"":" & IIf(mCaptureActive, "true", "false") & _
        ",""capture_path"":""" & Replace(mCapturePath, "\", "\\") & """}"
End Function

Public Function IsLogCaptureActive() As Boolean
    IsLogCaptureActive = mCaptureActive
End Function

Public Function LogCapturePath() As String
    LogCapturePath = mCapturePath
End Function

Public Sub StartLogCapture()
    On Error GoTo EH
    Dim folder As String
    Dim fso As Object
    folder = Environ$("LOCALAPPDATA") & "\GeoFooter\Logs"
    Set fso = CreateObject("Scripting.FileSystemObject")
    If Not fso.FolderExists(folder) Then fso.CreateFolder folder
    mCapturePath = folder & "\VBA_Capture_" & Format$(Now, "yyyymmdd_hhnnss") & ".txt"
    mCaptureActive = True
    WriteLogAudit "Log capture STARTED -> " & mCapturePath
    Exit Sub
EH:
...
Public Sub StopLogCapture()
    On Error Resume Next
    If mCaptureActive Then
        WriteLogAudit "Log capture STOPPED -> " & mCapturePath
    End If
    mCaptureActive = False
End Sub

' Called from DrainDiagCommands when the command blob embeds logging flags.
Public Sub ApplyLoggingJsonPublic(ByVal raw As String)
    On Error Resume Next
    ApplyLoggingJson raw
    mLoggingLoaded = True
    WriteLogAudit "Logging settings applied from diag command"
End Sub
