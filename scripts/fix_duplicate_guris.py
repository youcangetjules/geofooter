...
# Entry-script bootstrap: scripts/ is not a package; install root must be on path.
_ROOT = Path(__file__).resolve().parents[1]
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

from geofooter.paths import datastore_path  # noqa: E402
...
def fix_duplicates():
    ...
    db_path = str(datastore_path("guri_records.db"))
    ...
    # Backup next to the live DB
    print("Creating backup...")
    backup_path = (
        f"{db_path}.backup_duplicates_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    for record in mismatches:
        ...
        for attempt in range(max_attempts):
            new_guri = (
                f"{randhex(5)}x{randhex(5)}x{randhex(8)}x{randhex(8)}"
                f"x{randhex(3)}x{document_type}"
            )
            ...
            print(
                f"WARNING: Could not generate unique GURI for record {record_id} "
