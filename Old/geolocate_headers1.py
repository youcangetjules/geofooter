...
try:
    ...
    print(f"ERROR: Missing required module 'dnspython'. Install with: pip install dnspython")
...
try:
    ...
    print(f"ERROR: Missing required module 'python-whois'. Install with: pip install python-whois")
    print(f"Import error: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"ERROR: Missing required module 'requests'. Install with: pip install requests")
    print(f"Import error: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"ERROR: Missing required module 'pytz'. Install with: pip install pytz")
    print(f"Import error: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"ERROR: Missing required module 'pycountry'. Install with: pip install pycountry")
    print(f"Import error: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"ERROR: Missing required module 'ipwhois'. Install with: pip install ipwhois")
    print(f"Import error: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"ERROR: Missing required module 'guri'. Make sure guri.py is in the same directory.")
    print(f"Import error: {e}")
    sys.exit(1)
...
class HTMLReportGenerator:
    ...
    def _prepare_display_data(
        ...
        # Generate or retrieve GURI
        guri = "N/A"
        if guri_db and sender_email:
            try:
                # Extract recipients from headers if available
                recipients = "Unknown"
                if headers:
                ...
                # Extract subject from headers more reliably
                subject = "Unknown"
                if headers:
                    ...
                    if subject_match:
                        ...
                        # Fallback to first line if no Subject header found
                        subject = headers.split('\n')[0].strip()
                
                guri = guri_db.generate_guri(
                    sender_email, recipients, subject, dt_str, avg_risk, document_type="01"
                )
            except Exception as e:
...
def main():
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python geolocate_headers.py <header_file> [output_file]")
        sys.exit(1)
    ...
    print("Header file exists, proceeding with processing...")
    ...
    try:
        ...
        try:
            guri_db = GURIDatabase()
            logger.info("GURI database initialized successfully")
        except Exception as e:
        ...
        if output_file is None:
            ...
            print(f"Generated output file path: {output_file}")
        else:
            print(f"Using provided output file path: {output_file}")
        ...
        if os.path.exists(output_file):
            ...
            print(f"✓ File verification: {output_file} exists ({file_size} bytes)")
        else:
            print(f"✗ File verification failed: {output_file} does not exist")
