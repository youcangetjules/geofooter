...
' Layout (matches Home AES group): large ON/OFF + Short + Full, then a
' BeginGroup so Diagnostics / Settings / View Logs stack on the right.
...
' Force left-to-right: ON/OFF, Short, Full, Deep | Diagnostics, Settings, Logs
Private Sub OrderToolbarButtons(ByVal bar As CommandBar)
    ...
    If bar Is Nothing Then Exit Sub
    tags = Array(TAG_SERVICE, TAG_SHORTSCAN, TAG_FULLSCAN, TAG_DEEPSCAN, TAG_DIAGNOSTICS, TAG_SETTINGS, TAG_LOGS)
...
Private Function ResolveIconPath(ByVal fileName As String) As String
    ...
    paths = Array( _
        ...
        "C:\GeoFooter\icons\" & fileName, _
        "C:\GeoFooter\MSCAN\icons\" & fileName)
...
Public Sub ShortScanEmail()
    ...
    For i = 1 To explorer.Selection.Count
        ...
        If MSCANModule1.MailHasFooter(mail) Then
            hasFooter = hasFooter + 1
            MSCANModLogging.WriteLog "ShortScanEmail: Footer already present - " & mail.Subject
        Else
...
NextShortScanItem:
    ...
    MSCANModStatus.ShowStatus BuildShortScanStatusLine(completed, hasFooter, skipped)
    Exit Sub
...
Private Function BuildShortScanStatusLine(ByVal completed As Long, ByVal hasFooter As Long, ByVal skipped As Long) As String
    If completed = 0 And hasFooter = 0 And skipped > 0 Then
        BuildShortScanStatusLine = "Selection does not contain email messages."
        Exit Function
    End If

    If completed = 0 And hasFooter > 0 And skipped = 0 Then
        BuildShortScanStatusLine = "Selected email(s) already have an AES footer."
        Exit Function
    End If
    ...
    If hasFooter > 0 Then
        If Len(msg) > 0 Then msg = msg & "; "
        msg = msg & hasFooter & " already have footer"
    End If
