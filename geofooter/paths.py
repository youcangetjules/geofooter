...
Layout under the install root (relative):
  aes/         scan engine, scanners, dialogs, aes:// handler
  guri/        GURI core + desktop GUI
  aura/        data-broker removal (GURI tab)
  geofooter/   shared paths / version / crashlog
  VBA/         Outlook modules (.bas/.cls only)
  assets/      brand, icons, pages
  datastore/   local SQLite DBs + GURI lists
...
def _discover_from_file() -> Optional[Path]:
    ...
    for parent in [here.parent, *here.parents]:
        if (parent / "VERSION").is_file() and (
            (parent / "guri" / "gui.py").is_file()
            or (parent / "aes" / "geolocate_headers.py").is_file()
        ):
            return parent
    return None
