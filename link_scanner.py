...
# Email-platform click-tracking hosts (registrable domains). A redirect through
# one of these is only trusted when it decodes to the sender's own domain.
...
class LinkScanner:
    ...
    def _classify(
        ...
        if destination or any(h in path_q or h in host for h in REDIRECTOR_HINTS):
            ...
            host_ok = bool(sd and host_rd == sd) or host_rd in ESP_TRACKING_DOMAINS
            if dest_rd and sd and dest_rd == sd and host_ok:
                # Newsletter click tracking back to the sender's own site: not a risk signal.
                reasons.append(f"Tracking redirect to sender's own site ({dest_host})")
            elif dest_rd and sd:
                reasons.append(f"Redirects to {dest_host} (not the sender's domain)")
                risk = _max_risk(risk, "medium")
            elif dest_host:
                reasons.append(f"Possible redirector / tracking hop to {dest_host}")
                risk = _max_risk(risk, "medium")
            else:
                reasons.append("Possible redirector / tracking hop")
                risk = _max_risk(risk, "medium")
        ...
        if sender_domain:
            ...
            if sd and host and not (host == sd or host.endswith("." + sd)):
                # Brand lookalike in host while From domain differs
