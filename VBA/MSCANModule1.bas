...
' forceRescan is for explicit user action (the Short Scan button): it rescans a
' mail that already carries a footer and replaces the existing result.
Public Sub ProcessEmail(ByVal mail As Object, Optional ByVal forceRescan As Boolean = False)
    On Error GoTo EH
    ...
    If forceRescan Then
        MSCANModLogging.WriteLog "ProcessEmail: Starting async compact RESCAN for subject: " & SafeSubject(mail)
    Else
...
Public Sub ProcessCompleteFooter(ByVal mail As Object)
    ...
    Dim savedFilePath As String
    savedFilePath = ExportHeadersForGeolocation(mail, False, True)
...
Public Sub ProcessDeepScan(ByVal mail As Object)
    ...
    Dim savedFilePath As String
    savedFilePath = ExportHeadersForGeolocation(mail, True, True)
...
Public Sub CompleteAsyncFooter(ByVal jobId As String)
    ...
    Dim forceReplace As Boolean
    entryId = CStr(job("EntryID"))
    footerPath = CStr(job("FooterPath"))
    mode = CStr(job("Mode"))
    If job.Exists("Force") Then forceReplace = CBool(job("Force"))
...
Private Function ExportHeadersForGeolocation(ByVal mail As Object, _
    ...
    Optional ByVal heavyExport As Boolean = False) As String
    On Error GoTo EH
    ExportHeadersForGeolocation = ""

    MSCANModLogging.WriteLog "ExportHeadersForGeolocation: Starting for subject: " & SafeSubject(mail) & _
        " deep=" & CStr(deepMode) & " heavy=" & CStr(heavyExport)
    ...
    End If
    ' Compact: export a capped body for link/beacon counts (one HTMLBody read).
    ' Full/deep: full body + attachment export; beacon neutralize only on heavy.
    m_LastDeepPayloadDir = ""
    If heavyExport Or deepMode Then
        m_LastDeepPayloadDir = ExportDeepBodyPayload(mail, 0)
        beaconSummary = BuildBeaconSummary(mail, heavyExport)
    Else
        m_LastDeepPayloadDir = ExportDeepBodyPayload(mail, AES_COMPACT_BODY_CAP)
        beaconSummary = BuildBeaconSummaryFromPayload(m_LastDeepPayloadDir)
    End If
...
Private Function ExportDeepBodyPayload(ByVal mail As Object, Optional ByVal maxChars As Long = 0) As String
    On Error GoTo EH
    ...
    ' Rescans re-read a body we have already stamped; hand the scanner the
    ' original message only.
    htmlBody = StripAesMarkupFromBody(htmlBody)
    textBody = StripAesMarkupFromBody(textBody)

    If maxChars > 0 Then
        If Len(htmlBody) > maxChars Then htmlBody = Left$(htmlBody, maxChars)
        If Len(textBody) > maxChars Then textBody = Left$(textBody, maxChars)
    End If

    WriteTextUtf8 payloadDir & "\body.html", htmlBody
    WriteTextUtf8 payloadDir & "\body.txt", textBody

    MSCANModLogging.WriteLog "ExportDeepBodyPayload: Wrote body sidecars to " & payloadDir & _
        IIf(maxChars > 0, " (capped " & maxChars & ")", "")
...
Private Function BuildAttachmentSummary(ByVal mail As Object) As String
    ...
    For i = 1 To mail.Attachments.Count
        If Not IsAesBannerAttachment(mail.Attachments(i)) Then realCount = realCount + 1
    Next i
    BuildAttachmentSummary = "count=" & realCount & "; ok=0; not_ok=0"
End Function

' True for the hidden inline PNG that carries the AES status strip.
...
Private Function ExportAttachmentsForScan(ByVal mail As Object) As String
    ...
    For i = 1 To mail.Attachments.Count
        ...
        ' Our own status strip is an attachment; never scan it as sender content.
        If IsAesBannerAttachment(att) Then GoTo NextAttachment
...
Private Function BuildBeaconSummary(ByVal mail As Object, Optional ByVal neutralize As Boolean = True) As String
    On Error Resume Next
    ...
    If mail.BodyFormat = olFormatHTML Then
        ...
        If beaconCount > 0 And neutralize Then
    ...
    BuildBeaconSummary = FormatBeaconSummary(beaconCount, blockedCount, beaconUrls)
End Function

' Compact path: count beacons from the already-exported sidecar (no second HTMLBody read,
' never rewrite the message on auto-scan).
Private Function BuildBeaconSummaryFromPayload(ByVal payloadDir As String) As String
    On Error Resume Next
    Dim htmlBody As String
    Dim beaconCount As Long
    Dim beaconUrls As String

    BuildBeaconSummaryFromPayload = "count=0; status=None"
    If Len(payloadDir) = 0 Then Exit Function

    htmlBody = ReadTextUtf8(payloadDir & "\body.html")
    If Len(htmlBody) = 0 Then Exit Function

    beaconUrls = ""
    beaconCount = CollectEmailBeacons(htmlBody, beaconUrls)
    BuildBeaconSummaryFromPayload = FormatBeaconSummary(beaconCount, 0, beaconUrls)
End Function

Private Function FormatBeaconSummary(ByVal beaconCount As Long, ByVal blockedCount As Long, ByVal beaconUrls As String) As String
...
'===============================================================================
Private Function StartAsyncGeolocationJob(ByVal mail As Object, ByVal headerFilePath As String, ByVal footerMode As String, _
                                          Optional ByVal forceReplace As Boolean = False) As Boolean
    ...
    job("Mode") = footerMode
    ' Set only for user-initiated rescans, so an automatic scan can never
    ' overwrite a richer footer that is already on the mail.
    job("Force") = forceReplace
...
Private Function WriteAsyncGeoJobScript(ByVal vbsPath As String, ByVal workDir As String, ByVal commandLine As String, ByVal jobId As String, ByVal outputFile As String) As Boolean
    ...
    ts.WriteLine "End If"
    ' Walk the folder cursor instead of sorting: Items.Sort orders the whole
    ' folder (70k+ items) on Outlook's UI thread every pass, and the nudge only
    ' needs *some* uncached item to load. GetNext advances to a distinct item
    ' each iteration, which is what actually defeats the item cache.
    ts.WriteLine "' Load a different item each pass so ItemLoad fires (no folder sort)."
    ts.WriteLine "Set f = fso.OpenTextFile(" & VbsQuoteString(logPath) & ", 8, True)"
...
Private Function ApplyFooterToMail(ByVal mail As Object, ByVal footerPath As String, ByVal mode As String, _
                                   Optional ByVal forceReplace As Boolean = False) As Boolean
    Dim footerHTML As String
    footerHTML = ""
    On Error Resume Next
    footerHTML = ReadTextUtf8(footerPath)
    On Error GoTo 0
    ...
    End If
    If wantsMitigate Then
        MSCANModLogging.WriteLog "ApplyFooterToMail: high-risk mitigation for: " & SafeSubject(mail)
        ApplyFooterToMail = ApplyHighRiskTextOnlyMail(mail, footerPath, footerHTML)
        Exit Function
    End If
...
Private Function FooterRequestsMitigation(ByVal footerHTML As String) As Boolean
    FooterRequestsMitigation = (InStr(1, footerHTML, "<!-- AES-Mitigate: text-strip -->", vbTextCompare) > 0)
End Function

Private Function ExtractAesPlainSummary(ByVal footerHTML As String) As String
    Const START_M As String = "<!-- AES-Plain-Summary-Start -->"
    Const END_M As String = "<!-- AES-Plain-Summary-End -->"
    Dim s As Long, e As Long
    s = InStr(1, footerHTML, START_M, vbTextCompare)
    If s = 0 Then
        ExtractAesPlainSummary = ""
        Exit Function
    End If
    s = s + Len(START_M)
    e = InStr(s, footerHTML, END_M, vbTextCompare)
    If e = 0 Then
        ExtractAesPlainSummary = ""
        Exit Function
    End If
    ExtractAesPlainSummary = Trim$(Mid$(footerHTML, s, e - s))
End Function

' Score > 70: quarantine all attachments and rewrite the mail as plain text
' with an AES mitigation notice and a short scan summary (full HTML report
' remains on disk; its path is in the plain summary when available).
' Original HTML is saved under mitigated_html\ so aes://restore-html can
' put the message back into HTML format.
Private Function ApplyHighRiskTextOnlyMail(ByVal mail As Object, ByVal footerPath As String, _
                                           ByVal footerHTML As String) As Boolean
    On Error GoTo ErrHandler
    ApplyHighRiskTextOnlyMail = False
    ...
    summary = ExtractAesPlainSummary(footerHTML)
    If Len(summary) = 0 Then
        summary = "Risk score exceeded 70." & vbCrLf & _
                  "Mitigation: attachments quarantined; message converted to text-only."
    End If

    ' Fill restore-html id (or drop that action if backup failed).
    If Len(restoreId) > 0 Then
        summary = Replace(summary, "{{AES_RESTORE_ID}}", restoreId, 1, -1, vbTextCompare)
    Else
        summary = StripPlainActionBlock(summary, "Restore original HTML format")
    End If

    Dim notice As String
    notice = "AES HIGH RISK MITIGATION" & vbCrLf & _
             "Risk score exceeded 70. Attachments quarantined: " & CStr(removed) & _
             ". This message was converted to text-only." & vbCrLf & vbCrLf & _
    ...
    MSCANModLogging.WriteLog "ApplyHighRiskTextOnlyMail: mitigated subject=" & SafeSubject(mail) & _
        " attachmentsQuarantined=" & CStr(removed) & " restoreId=" & restoreId

    On Error Resume Next
    Dim fso As Object
    Set fso = CreateObject("Scripting.FileSystemObject")
    If Len(footerPath) > 0 Then fso.DeleteFile footerPath
    Err.Clear
...
ErrHandler:
    MSCANModLogging.WriteLog "ApplyHighRiskTextOnlyMail error: #" & Err.Number & " - " & Err.Description
End Function

' Write original HTML + a small JSON sidecar; return short id for aes://restore-html?id=
Private Function SaveMitigatedHtmlBackup(ByVal mail As Object, ByVal htmlBody As String, _
                                         ByRef outPath As String) As String
    On Error GoTo EH
    SaveMitigatedHtmlBackup = ""
    outPath = ""
    If Len(Trim$(htmlBody)) = 0 Then Exit Function

    Dim dir As String
    dir = GetBaseDir() & "\mitigated_html"
    If Not EnsureFolderExists(dir) Then Exit Function

    Dim rid As String
    rid = "r" & Format(Now, "yyyymmddhhnnss") & Right$("000" & CStr(Int((Timer - Int(Timer)) * 1000)), 3)

    Dim htmlPath As String
    htmlPath = dir & "\" & rid & ".html"
    WriteTextUtf8 htmlPath, htmlBody
    ...
    storeId = ""
    On Error Resume Next
    entryId = CStr(mail.EntryID)
    storeId = CStr(mail.Parent.StoreID)
    Err.Clear
    On Error GoTo EH
    ...
    meta = "{" & _
           ...
           """store_id"":""" & JsonEscape(storeId) & """," & _
           """html_path"":""" & JsonEscape(htmlPath) & """," & _
           """subject"":""" & JsonEscape(SafeSubject(mail)) & """" & _
           "}"
    WriteTextUtf8 dir & "\" & rid & ".json", meta
    ...
    If Not ups Is Nothing Then
        Set up = ups.Find("AESHtmlBackup")
        If up Is Nothing Then Set up = ups.Add("AESHtmlBackup", 1)
        If Not up Is Nothing Then up.Value = rid
    End If
    Err.Clear

    outPath = htmlPath
    SaveMitigatedHtmlBackup = rid
    Exit Function
EH:
    MSCANModLogging.WriteLog "SaveMitigatedHtmlBackup error: #" & Err.Number & " - " & Err.Description
End Function
...
Private Function TryRestoreMitigatedHtml(ByVal mail As Object) As Boolean
    ...
    If Not TypeOf mail Is Outlook.MailItem Then Exit Function

    Dim bodyProbe As String
    bodyProbe = ""
    On Error Resume Next
    bodyProbe = CStr(mail.Body)
    If Len(bodyProbe) < 20 Then bodyProbe = bodyProbe & CStr(mail.HTMLBody)
    Err.Clear
    On Error GoTo EH

    Dim looksMitigated As Boolean
    looksMitigated = (InStr(1, bodyProbe, "AES HIGH RISK MITIGATION", vbTextCompare) > 0) Or _
                     (InStr(1, bodyProbe, "converted to text-only", vbTextCompare) > 0) Or _
                     (InStr(1, bodyProbe, "restore-html?id=", vbTextCompare) > 0)

    Dim rid As String
    rid = ExtractRestoreHtmlId(bodyProbe)
    ...
    If Not ups Is Nothing Then
        Set up = ups.Find("AESHtmlBackup")
        If Not up Is Nothing And Len(rid) = 0 Then rid = Trim$(CStr(up.Value))
        If Not up Is Nothing And Len(Trim$(CStr(up.Value))) > 0 Then looksMitigated = True
    End If
    Err.Clear
    On Error GoTo EH

    If Not looksMitigated And Len(rid) = 0 Then
        MSCANModLogging.WriteLog "TryRestoreMitigatedHtml: skip (not a mitigated body) subject=" & SafeSubject(mail)
        Exit Function
    End If
    ...
    If Not fso.FolderExists(dir) Then
        MSCANModLogging.WriteLog "TryRestoreMitigatedHtml: no mitigated_html folder"
        Exit Function
    End If
    ...
    If Len(rid) > 0 Then
        ...
        If Not fso.FileExists(htmlPath) Then
            MSCANModLogging.WriteLog "TryRestoreMitigatedHtml: backup missing for rid=" & rid
            htmlPath = ""
        End If
    End If

    If Len(htmlPath) = 0 Then
        htmlPath = FindMitigatedHtmlBackup(mail, dir, rid)
    End If

    If Len(htmlPath) = 0 Then
        MSCANModLogging.WriteLog "TryRestoreMitigatedHtml: no backup matched subject=" & SafeSubject(mail)
        Exit Function
    End If

    Dim htmlBody As String
    htmlBody = ReadTextUtf8(htmlPath)
    If Len(Trim$(htmlBody)) = 0 Then Exit Function
    ...
    If Not CommitMailHtml(mail, entryId, htmlBody) Then
        MSCANModLogging.WriteLog "TryRestoreMitigatedHtml: CommitMailHtml failed rid=" & rid
        Exit Function
    End If

    MSCANModLogging.WriteLog "TryRestoreMitigatedHtml: restored rid=" & rid & " path=" & htmlPath & " subject=" & SafeSubject(mail)
    TryRestoreMitigatedHtml = True
    Exit Function
EH:
    MSCANModLogging.WriteLog "TryRestoreMitigatedHtml error: #" & Err.Number & " - " & Err.Description
End Function

Private Function ExtractRestoreHtmlId(ByVal text As String) As String
    On Error Resume Next
    ExtractRestoreHtmlId = ""
    Dim p As Long, e As Long, ch As String, i As Long
    p = InStr(1, text, "restore-html?id=", vbTextCompare)
    If p = 0 Then Exit Function
    p = p + Len("restore-html?id=")
    e = p
    For i = p To Len(text)
        ch = Mid$(text, i, 1)
        If ch Like "[0-9A-Za-z_]" Or ch = "-" Then
            e = i
        Else
            Exit For
        End If
    Next i
    If e >= p Then ExtractRestoreHtmlId = Mid$(text, p, e - p + 1)
End Function

Private Function FindMitigatedHtmlBackup(ByVal mail As Object, ByVal dir As String, ByRef outRid As String) As String
    On Error GoTo EH
    FindMitigatedHtmlBackup = ""
    outRid = ""
    Dim fso As Object
    Set fso = CreateObject("Scripting.FileSystemObject")
    Dim entryId As String
    Dim subj As String
    entryId = ""
    subj = LCase$(Trim$(SafeSubject(mail)))
    On Error Resume Next
    entryId = CStr(mail.EntryID)
    Err.Clear
    On Error GoTo EH
    ...
    For Each file In folder.Files
        If LCase$(fso.GetExtensionName(file.Name)) = "json" Then
            Dim meta As String
            meta = ReadTextUtf8(file.Path)
            Dim matched As Boolean
            matched = False
            If Len(entryId) > 0 And InStr(1, meta, entryId, vbBinaryCompare) > 0 Then matched = True
            If Not matched And Len(subj) > 8 Then
                ' JSON subject is escaped; compare a stable prefix of the live subject.
                Dim needle As String
                needle = Left$(subj, 40)
                If InStr(1, LCase$(meta), needle, vbTextCompare) > 0 Then matched = True
            End If
            If matched Then
                Dim candRid As String
                Dim candHtml As String
                candRid = fso.GetBaseName(file.Name)
                candHtml = dir & "\" & candRid & ".html"
                If fso.FileExists(candHtml) Then
                    If file.DateLastModified >= bestTs Then
                        bestTs = file.DateLastModified
                        FindMitigatedHtmlBackup = candHtml
                        outRid = candRid
                    End If
                End If
            End If
...
Private Sub FindAesFooterBounds(ByVal bodyHtml As String, ByRef startPos As Long, ByRef endPos As Long, ByRef endLen As Long, _
    ...
    Const ANCHOR_START As String = "id='aes-footer-start'"
    Const ANCHOR_START2 As String = "id=""aes-footer-start"""
    Const ANCHOR_END As String = "id='aes-footer-end'"
    Const ANCHOR_END2 As String = "id=""aes-footer-end"""
    Const VISIBLE_START As String = "Aliniant AES Scan Result"
    ...
    End If
    If startPos = 0 Then
        startPos = InStr(1, bodyHtml, ANCHOR_START, vbTextCompare)
        If startPos = 0 Then startPos = InStr(1, bodyHtml, ANCHOR_START2, vbTextCompare)
        If startPos > 0 Then
            ' Rewind to the opening '<' of the anchor tag.
            Dim lt As Long
            lt = InStrRev(Left$(bodyHtml, startPos), "<")
            If lt > 0 Then startPos = lt
            endMarker = ""
        End If
    End If
    If startPos = 0 Then
        startPos = InStr(1, bodyHtml, VISIBLE_START, vbTextCompare)
        If startPos > 0 Then
            Dim lt2 As Long
            lt2 = InStrRev(Left$(bodyHtml, startPos), "<div")
            If lt2 = 0 Then lt2 = InStrRev(Left$(bodyHtml, startPos), "<")
            If lt2 > 0 Then startPos = lt2
            endMarker = ""
        End If
    End If
    If startPos = 0 Then Exit Sub

    If Len(endMarker) > 0 Then
        endPos = InStr(startPos, bodyHtml, endMarker, vbTextCompare)
        If endPos > 0 Then endLen = Len(endMarker)
    End If
    If endPos = 0 Then
        endPos = InStr(startPos, bodyHtml, ANCHOR_END, vbTextCompare)
        If endPos = 0 Then endPos = InStr(startPos, bodyHtml, ANCHOR_END2, vbTextCompare)
        If endPos > 0 Then
            Dim ltEnd As Long
            Dim gt As Long
            ltEnd = InStrRev(Left$(bodyHtml, endPos), "<")
            If ltEnd > 0 Then endPos = ltEnd
            gt = InStr(endPos, bodyHtml, ">")
            If gt > 0 Then
                endLen = gt - endPos + 1
            Else
                endLen = 1
    ...
    If endPos = 0 And allowBodyFallback Then
        ...
        If endPos > 0 Then
            endLen = 0  ' replace up to but not including </body>
        End If
    End If
End Sub
...
' Remove one "  Label:" / "    url" pair from the plain AES actions block.
Private Function StripPlainActionBlock(ByVal summary As String, ByVal label As String) As String
    On Error Resume Next
    Dim needle As String
    Dim p As Long
    Dim lineEnd As Long
    ...
    needle = "  " & label & ":"
    p = InStr(1, summary, needle, vbTextCompare)
    If p = 0 Then
        StripPlainActionBlock = summary
        Exit Function
    End If
    ' Drop label line + following indented URL line.
    lineEnd = InStr(p, summary, vbCrLf)
    If lineEnd = 0 Then
        StripPlainActionBlock = Left$(summary, p - 1)
        Exit Function
    End If
    nextLine = lineEnd + 2
    If nextLine <= Len(summary) And Mid$(summary, nextLine, 4) = "    " Then
        lineEnd = InStr(nextLine, summary, vbCrLf)
        If lineEnd = 0 Then
            StripPlainActionBlock = Left$(summary, p - 1)
            Exit Function
        End If
    End If
    StripPlainActionBlock = Left$(summary, p - 1) & Mid$(summary, lineEnd + 2)
End Function
...
Private Function StripAesTopBannersFromBody(ByVal bodyHtml As String) As String
    ...
    StripAesTopBannersFromBody = StripBlockBetween(bodyHtml, _
        "<!-- AES Top Banners Start -->", "<!-- AES Top Banners End -->")
End Function

' Defang hrefs listed in <!-- AES-RiskTable-Disable: url1|url2 --> from the footer payload.
Private Function DisableRiskTableLinksInHtml(ByVal bodyHtml As String, ByVal footerHTML As String) As String
    On Error Resume Next
    DisableRiskTableLinksInHtml = bodyHtml
    Const MARK As String = "<!-- AES-RiskTable-Disable: "
    ...
    p = InStr(1, footerHTML, MARK, vbTextCompare)
    If p = 0 Then Exit Function
    p = p + Len(MARK)
    e = InStr(p, footerHTML, "-->")
    If e <= p Then Exit Function
    list = Trim$(Mid$(footerHTML, p, e - p))
    ...
    For i = LBound(parts) To UBound(parts)
        u = Trim$(parts(i))
        u = Replace(u, "%7C", "|")
        If Len(u) = 0 Then GoTo NextUrl
        If LCase$(Left$(u, 7)) = "http://" Or LCase$(Left$(u, 8)) = "https://" Or LCase$(Left$(u, 4)) = "www." Then
            inert = "aes://blocked-link"
            bodyHtml = Replace(bodyHtml, "href=""" & u & """", "href=""" & inert & """ title=""AES: disabled - found on RiskTable""", 1, -1, vbTextCompare)
            bodyHtml = Replace(bodyHtml, "href='" & u & "'", "href='" & inert & "' title='AES: disabled - found on RiskTable'", 1, -1, vbTextCompare)
            bodyHtml = Replace(bodyHtml, "href=" & u, "href=" & inert, 1, -1, vbTextCompare)
        End If
NextUrl:
    Next i
    DisableRiskTableLinksInHtml = bodyHtml
...
' Removes every marker-delimited block, markers included.
Private Function StripBlockBetween(ByVal src As String, ByVal startMarker As String, _
                                   ByVal endMarker As String) As String
    On Error Resume Next
    Dim s As Long, e As Long
    Do
        s = InStr(1, src, startMarker, vbTextCompare)
        If s = 0 Then Exit Do
        e = InStr(s, src, endMarker, vbTextCompare)
        If e = 0 Then Exit Do
        e = e + Len(endMarker)
        src = Left$(src, s - 1) & Mid$(src, e)
    Loop
    StripBlockBetween = src
End Function

' Strips AES's own footer and status strip from a body before it is handed to
' the scanner. Without this a rescan counts our injected report links and
' status image as sender content and inflates the risk score.
...
'
' Deliberately saves nothing. The caller performs one Save after setting the
' body, which commits the attachment and the markup together; saving here as
' well opened two extra windows for "the message has been changed" on an item
' the reading pane already holds, and a failure part-way then poisoned the
' body write too, so the whole footer was lost rather than just the image.
Private Function EmbedAesBannerImage(ByVal mail As Object, ByVal bannerHtml As String) As String
    ...
    stage = "remove previous banner parts"
    ...
    ' here has to drop the part and leave the file:// markup alone.
    stage = "set Content-ID"
    att.PropertyAccessor.SetProperty PR_ATTACH_CONTENT_ID, AES_BANNER_CID

    ' Presentation only, and PR_ATTACH_FLAGS is read-only on some stores
    ' (refused on this profile), so the image must still resolve without them.
    On Error Resume Next
    att.PropertyAccessor.SetProperty PR_ATTACH_MIME_TAG, "image/png"
    att.PropertyAccessor.SetProperty PR_ATTACH_FLAGS, ATT_MHTML_REF
...
ErrHandler:
    MSCANModLogging.WriteLog "EmbedAesBannerImage failed at '" & stage & "': #" & _
        Err.Number & " - " & Err.Description & " (falling back to file:// markup)"
    ' Leave no orphan part behind for the caller's Save to commit.
    On Error Resume Next
...
Private Function InsertFooterIntoMail(mail As Object, footerPath As String) As Boolean
    ...
    If TypeOf mail Is Outlook.MailItem Then
        ...
        Dim entryIdIns As String
        entryIdIns = ""
        On Error Resume Next
        entryIdIns = CStr(mail.EntryID)
        Err.Clear
        On Error GoTo ErrHandler
        ...
        Dim bannerForIns As String
        bannerForIns = bannerHtml
        If Len(bannerForIns) > 0 Then bannerForIns = EmbedAesBannerImage(mail, bannerForIns)
        ...
        If Not CommitMailHtml(mail, entryIdIns, finalIns) Then
            ...
            If mail Is Nothing Then
                ...
                Exit Function
            End If
            finalIns = InjectAesTopBanner(newBody, bannerHtml)
            If Not CommitMailHtml(mail, entryIdIns, finalIns) Then
                MSCANModLogging.WriteLog "InsertFooterIntoMail: CommitMailHtml failed for subject: " & SafeSubject(mail)
                InsertFooterIntoMail = False
                Exit Function
            End If
        End If
...
Private Function ReplaceAesFooterInMail(ByVal mail As Object, ByVal footerPath As String) As Boolean
    ...
    bannerForBody = bannerHtml
    If Len(bannerForBody) > 0 Then bannerForBody = EmbedAesBannerImage(mail, bannerForBody)
    ...
    If Not CommitMailHtml(mail, entryId, finalHtml) Then
        ...
        If mail Is Nothing Then
            ...
            Exit Function
        End If
        finalHtml = InjectAesTopBanner(replacedBody, bannerHtml)
        If Not CommitMailHtml(mail, entryId, finalHtml) Then
            MSCANModLogging.WriteLog "ReplaceAesFooterInMail: CommitMailHtml failed for subject: " & SafeSubject(mail)
            ReplaceAesFooterInMail = False
            Exit Function
        End If
    End If
    ...
    MarkAesScanned mail
    MSCANModLogging.WriteLog "ReplaceAesFooterInMail: AES footer replaced for subject: " & SafeSubject(mail)
    On Error Resume Next
    fso.DeleteFile footerPath
    On Error GoTo ErrHandler
    ReplaceAesFooterInMail = True
